...
def _probe_embedding_support(model: str, token: str, embeddings_endpoint: str) -> bool:
    ...
    print(f"  [budget] Embeddings probe ({model}): {support_text}")
    return supported
...
def _execute_github_chat_completion(
    ...
    try:
        ...
        error_body = err.read().decode("utf-8", errors="replace")
        print("Error: GitHub Models chat completion failed.")
        print("  Operation: final release-announcement generation call")
        ...
        print(
            "Error: GitHub Models chat completion request failed before "
...
def _summarize_pr_chunks(
    ...
    print(f"  [budget] Chat-only: summarising PR in {len(parts_to_summarize)} chunks…")
    condensed = "\n\n".join(
...
def call_github_models_api(  # pylint: disable=too-many-arguments,too-many-positional-arguments,too-many-locals
    ...
    if model_override and not chat_model_override and not embedding_model_override:
        if "embedding" in model_override.lower():
            embedding_model = model_override
        else:
