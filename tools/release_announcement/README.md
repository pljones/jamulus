...
- `--pipeline`: One of `legacy` (default) or `staged`.
...
- Regression test skipped for missing baseline:
  - Ensure baseline artifacts exist under `.vscode/release-announcement-baseline/`.
...
- Matrix regression fails with output mismatches:
  - Compare generated artifacts under `build/release_announcement/matrix_regression/`.
