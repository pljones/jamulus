...
build_app() {
    ...
    rm -rf "${build_dir}"
    mkdir -p "${build_dir}"
    if [[ "$build_mode" == release && (-n "${JAMULUS_ANDROID_KEYSTORE:-}" ||
        -n "${JAMULUS_ANDROID_KEYSTORE_BASE64:-}")  ]]; then
        BUILD_DIR="$build_dir"
        prepare_keystore
    else
        BUILD_DIR="$build_dir"
    ...
    [[ "$build_mode" == release ]] && package_args+=(--release)
    if [[ "$build_mode" == release && -n "${ANDROID_KEYSTORE_FILE:-}" ]]; then
